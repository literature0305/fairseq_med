...
class SegmentLevelTraining(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True, update_num=None, hypos=None):
        ...
        if hypos is not None and self.training:
            ...
            # constant value
            ter_threshold = 0.50 # (BLEU 35.02) # TODO: remove hard coded tuning factor2
            ...
            max_len_hyp = 0
            to_print_over_threshold_rate=0
            ...
            for idx in range(len(hypos)):
                ...
                if len(accepted_hyps) == 0:
                    prev_output_tokens_new[idx,:,1:len(list_true)+1] = torch.tensor(list_true).to(prev_output_tokens_new.device)
                else:
                    to_print_over_threshold_rate = to_print_over_threshold_rate + 1
                    for idx_hyp_tmp in range(len(hypos[idx])):
                        idx_hyp = idx_hyp_tmp % len(accepted_hyps)
                        prev_output_tokens_new[idx][idx_hyp_tmp][1:len(accepted_hyps[idx_hyp])+1] = torch.tensor(accepted_hyps[idx_hyp]).to(prev_output_tokens_new.device)
        ...
        if hypos is not None and self.training:
            ...
            loss, nll_loss = self.compute_loss_bleu4(model, net_output, sample, prev_output_tokens_new, reduce=reduce, bleu_size=bleu_size_random+2)
    ...
    def compute_loss_bleu4(self, model, net_output, sample, prev_output_tokens_new, reduce=True, bleu_size=4):
